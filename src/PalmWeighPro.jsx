...
const T = {
  ...
  accent:"#22d3a5", amber:"#f59e0b", red:"#ef4444", blue:"#3b82f6", purple:"#8b5cf6",
  text:"#e2e8f0", mid:"#94a3b8", dim:"#475569",
};
...
const SUPPLIERS0 = [
  { id:"sup1", name:"Johor Palm Resources Sdn Bhd", code:"JPR", address:"Johor Bahru, Johor",  contact:"0197001001", active:true },
  { id:"sup2", name:"Southern Agro Trading",         code:"SAT", address:"Batu Pahat, Johor",   contact:"0197001002", active:true },
  { id:"sup3", name:"Green Harvest Plantation",      code:"GHP", address:"Kluang, Johor",       contact:"0197001003", active:true },
  { id:"sup4", name:"Evergreen Palm Trading",        code:"EPT", address:"Segamat, Johor",      contact:"0197001004", active:true },
  { id:"sup5", name:"Maju Palm Industries",          code:"MPI", address:"Mersing, Johor",      contact:"0197001005", active:true },
];
const DRIVERS0 = [
  { id:"d1", name:"Ahmad bin Yusof",    ic:"850312-01-5001", phone:"0197101001", rfid:"RF001", license:"GDL-001", stationId:"s1", active:true },
  { id:"d2", name:"Azman bin Harun",    ic:"880605-01-4002", phone:"0197101002", rfid:"RF002", license:"GDL-002", stationId:"s1", active:true },
  { id:"d3", name:"Kumar Selvam",       ic:"790820-01-3003", phone:"0197101003", rfid:"RF003", license:"GDL-003", stationId:"s2", active:true },
  { id:"d4", name:"Raj Gopal",          ic:"820114-01-2004", phone:"0197101004", rfid:"RF004", license:"GDL-004", stationId:"s2", active:true },
  { id:"d5", name:"Lee Chee Wai",       ic:"910930-01-1005", phone:"0197101005", rfid:"",      license:"GDL-005", stationId:"s3", active:true },
];
const VEHICLES0 = [
  { id:"v1", plate:"JPV 1234", type:"Lorry",  tare:8200,  rfid:"RFV001", driverId:"d1", stationId:"s1", active:true },
  { id:"v2", plate:"JQK 5678", type:"Lorry",  tare:7800,  rfid:"RFV002", driverId:"d2", stationId:"s1", active:true },
  { id:"v3", plate:"JRU 8899", type:"Truck",  tare:12400, rfid:"RFV003", driverId:"d3", stationId:"s2", active:true },
  { id:"v4", plate:"JSD 1122", type:"Lorry",  tare:8000,  rfid:"RFV004", driverId:"d4", stationId:"s2", active:true },
  { id:"v5", plate:"JTK 3344", type:"Pickup", tare:2800,  rfid:"",       driverId:"d5", stationId:"s3", active:true },
];
const USERS0 = [
  { id:"u1", username:"admin",    password:"admin",  name:"Administrator",   role:"admin"    },
  { id:"u2", username:"operator", password:"1234",   name:"Weighbridge Operator", role:"operator" },
  { id:"u3", username:"demo",     password:"demo",   name:"Demo User (UAT)", role:"operator" },
];
...
function makeTx(i) {
  ...
  return {
    id: sta.prefix + "-" + String(i + 1).padStart(6, "0"),
    stationId: sta.id, stationPrefix: sta.prefix,
...
function useLS(key, init) {
  ...
  const set = useCallback(function(fn) {
...
// ---- ICON -------------------------------------------------------------------
function Ic(props) {
  var d = props.d, size = props.size || 16, color = props.color || T.mid;
  return (
...
var IC = {
  ...
  queue:     "M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 0 2-2h2a2 2 0 0 0 2 2",
  ...
  vehicle:   "M1 3h15l3 9H1zM1 17a2 2 0 1 0 4 0 2 2 0 0 0-4 0zm13 0a2 2 0 1 0 4 0 2 2 0 0 0-4 0z",
...
// ---- SHARED UI --------------------------------------------------------------
var cardStyle = { background:T.card, border:"1px solid " + T.border, borderRadius:12, padding:20 };

function Card(props) {
  return <div style={Object.assign({}, cardStyle, props.style || {})}>{props.children}</div>;
}

function Badge(props) {
  var label = props.label, color = props.color || T.accent;
  return (
    <span style={{
      background: color + "22", color: color,
      border: "1px solid " + color + "44",
      borderRadius:6, padding:"2px 8px", fontSize:11, fontWeight:700,
      letterSpacing:0.5, fontFamily:"IBM Plex Mono,monospace", whiteSpace:"nowrap",
    }}>{label}</span>
  );
}

function Btn(props) {
  var label = props.label, icon = props.icon, onClick = props.onClick;
  var variant = props.variant || "primary";
  var small = props.small || false;
  var disabled = props.disabled || false;
  var full = props.full || false;
  var bgMap = { primary:T.accent, danger:T.red, amber:T.amber, blue:T.blue, ghost:T.border };
  ...
  return (
    <button
      ...
      disabled={disabled}
      style={{
        background: bg, color: disabled ? T.dim : fg,
        border:"none", borderRadius:8,
        padding: small ? "5px 10px" : "8px 16px",
        fontSize: small ? 11 : 13, fontWeight:700,
        cursor: disabled ? "not-allowed" : "pointer",
        display:"inline-flex", alignItems:"center", gap:6,
        letterSpacing:0.3, fontFamily:"IBM Plex Mono,monospace",
...
function FInput(props) {
  ...
  var type = props.type || "text", placeholder = props.placeholder || "";
  var options = props.options, required = props.required, style = props.style || {};
  return (
    <div style={Object.assign({ display:"flex", flexDirection:"column", gap:4 }, style)}>
      {label && (
        <label style={{ fontSize:11, color:T.mid, fontWeight:600, letterSpacing:0.5, textTransform:"uppercase" }}>
          {label}{required ? " *" : ""}
        </label>
      )}
      {options ? (
        <select value={value} onChange={function(e) { onChange(e.target.value); }} style={IS}>
...
function StatCard(props) {
  ...
  var color = props.color || T.accent, icon = props.icon;
  return (
    <Card style={{ display:"flex", flexDirection:"column", gap:8 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <span style={{ fontSize:10, color:T.mid, fontWeight:600, textTransform:"uppercase", letterSpacing:0.5 }}>{label}</span>
        {icon && <Ic d={IC[icon]} size={15} color={color} />}
      </div>
      <div style={{ fontSize:26, fontWeight:800, color:color, fontFamily:"IBM Plex Mono,monospace", letterSpacing:-1 }}>{value}</div>
...
function DataTable(props) {
  var cols = props.cols, rows = props.rows, emptyMsg = props.emptyMsg || "No records";
  return (
    <div style={{ overflowX:"auto" }}>
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
        <thead>
          <tr style={{ borderBottom:"1px solid " + T.border }}>
            {cols.map(function(c) {
              return (
                <th key={c.key} style={{
                  padding:"7px 10px", textAlign:"left", color:T.dim,
                  fontWeight:600, letterSpacing:0.5, fontSize:10,
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr>
              <td colSpan={cols.length} style={{ padding:32, textAlign:"center", color:T.dim }}>{emptyMsg}</td>
            </tr>
            ...
            return (
              ...
              >
                {cols.map(function(c) {
                  return (
                    <td key={c.key} style={{ padding:"8px 10px", color:T.text, whiteSpace:"nowrap" }}>
                      {c.render ? c.render(row[c.key], row) : (row[c.key] != null ? row[c.key] : "-")}
                    </td>
                  );
                })}
              </tr>
...
function TruckSVG(props) {
  ...
  return (
    <svg width="260" height="64" viewBox="0 0 260 64" style={{ overflow:"hidden" }}>
      ...
      <g transform={"translate(" + truckX + ",0)"}>
        <rect x="88" y="16" width="30" height="30" rx="3" fill={c} opacity="0.9" />
        <rect x="90" y="18" width="12" height="10" rx="2" fill="#000" opacity="0.4" />
        <rect x="0" y="20" width="92" height="26" rx="2" fill={c} opacity="0.7" />
              ...
              {spokeAngles.map(function(a) {
                ...
                return (
                  <line key={a} x1="0" y1="0"
                    x2={Math.cos(rad) * 4.5} y2={Math.sin(rad) * 4.5}
...
function ScaleSim(props) {
  ...
  useEffect(function() {
    ...
    function tick(now) {
      ...
      if (ph === PH.APPROACHING) {
        ...
        setDisp(w2);
        setBars(function(p) { var n = p.slice(1); n.push(w2); return n; });
        if (el >= S) {
        ...
        setBars(function(p) { var n = p.slice(1); n.push(targetWeight); return n; });
  ...
  var seg = phase === PH.CAPTURED ? T.accent : phase >= PH.SETTLING ? T.amber : T.dim;
  ...
  return (
    <Card style={{ background:"#0a0f1a" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        <div>
          <div style={{ fontSize:9, color:T.dim, letterSpacing:2, textTransform:"uppercase", marginBottom:1 }}>Weighbridge Terminal</div>
          <div style={{ fontSize:12, fontWeight:700, color:T.text, fontFamily:"IBM Plex Mono,monospace" }}>{label}</div>
        </div>
        <Badge label={phLbl[phase]} color={phCol[phase]} />
      </div>

      <div style={{ background:"#050a10", borderRadius:8, padding:"4px 0", marginBottom:8, overflow:"hidden" }}>
        <TruckSVG truckX={truckX} active={phase >= PH.SETTLING} spk={spk} />
      </div>

      <div style={{
        background:"#050a10", border:"2px solid " + seg + "44", borderRadius:10,
        padding:"10px 14px", display:"flex", gap:2, alignItems:"center",
        justifyContent:"center", marginBottom:8,
      }}>
        {digs.map(function(d, i) {
          return (
            <span key={i} style={{
              fontFamily:"IBM Plex Mono,monospace", fontSize:32, fontWeight:900,
              color:seg, minWidth:20, textAlign:"center",
            }}>{d}</span>
          );
        })}
        <span style={{ fontSize:12, color:T.dim, marginLeft:8, fontFamily:"IBM Plex Mono,monospace" }}>kg</span>
      </div>

      <div style={{
        display:"flex", gap:1, height:30, alignItems:"flex-end",
        margin:"6px 0", background:"#050a10", borderRadius:6, padding:"3px 4px",
      }}>
        {bars.map(function(b, i) {
          return (
            <div key={i} style={{
              flex:1, height:((b / bmax) * 100) + "%", minHeight:2,
              background: i === 49 ? T.accent : T.dim + "44", borderRadius:1,
            }} />
          );
        })}
      </div>
      ...
      <div style={{ fontSize:9, color:T.dim, textAlign:"center", marginBottom:10, letterSpacing:1, textTransform:"uppercase" }}>
        {statusLabel}
      </div>

      <div style={{ display:"flex", gap:8 }}>
        {phase === PH.IDLE && (
          <Btn label="START WEIGHING" icon="scale" onClick={startWeigh} full={true} />
        )}
        {phase === PH.CAPTURED && (
          <Btn label="RESET" icon="refresh" onClick={resetScale} variant="amber" />
        )}
        {phase > PH.IDLE && phase < PH.CAPTURED && (
          <Btn label="CANCEL" icon="x" onClick={resetScale} variant="danger" small={true} />
        )}
      </div>
    </Card>
  );
...
function Login(props) {
  var users = props.users, onLogin = props.onLogin;
  var [uname, setUname] = useState("");
  var [pass, setPass] = useState("");
  var [err, setErr] = useState("");

  function handle() {
    var u = users.find(function(x) { return x.username === uname && x.password === pass; });
    ...
    <div style={{ minHeight:"100vh", background:T.bg, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ width:360 }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{ fontSize:28, fontWeight:900, color:T.accent, fontFamily:"IBM Plex Mono,monospace", letterSpacing:-1 }}>PALMWEIGH</div>
          <div style={{ fontSize:11, color:T.dim, letterSpacing:2, marginTop:2 }}>WEIGHBRIDGE MANAGEMENT SYSTEM</div>
          <div style={{ fontSize:10, color:T.dim, marginTop:4 }}>{COMPANY}</div>
          <div style={{ marginTop:8, display:"inline-block", background:T.amber + "22", border:"1px solid " + T.amber + "44", borderRadius:6, padding:"3px 10px", fontSize:10, color:T.amber, fontFamily:"IBM Plex Mono,monospace", letterSpacing:0.5 }}>UAT Demonstration Version</div>
        </div>
        <Card>
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
              <label style={{ fontSize:11, color:T.mid, fontWeight:600, letterSpacing:0.5, textTransform:"uppercase" }}>Username</label>
              <div style={{ position:"relative" }}>
                <div style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)" }}>
                  <Ic d={IC.user} size={14} color={T.dim} />
                </div>
                <input value={uname} onChange={function(e) { setUname(e.target.value); }}
                  onKeyDown={function(e) { if (e.key === "Enter") handle(); }}
                  placeholder="Enter username" style={Object.assign({}, IS, { paddingLeft:32 })} />
              </div>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
              <label style={{ fontSize:11, color:T.mid, fontWeight:600, letterSpacing:0.5, textTransform:"uppercase" }}>Password</label>
              <div style={{ position:"relative" }}>
                <div style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)" }}>
                ...
                <input type="password" value={pass} onChange={function(e) { setPass(e.target.value); }}
                  onKeyDown={function(e) { if (e.key === "Enter") handle(); }}
                  placeholder="Enter password" style={Object.assign({}, IS, { paddingLeft:32 })} />
              </div>
            </div>
            {err && (
              <div style={{ fontSize:12, color:T.red, padding:"6px 10px", background:T.red + "11", borderRadius:6, border:"1px solid " + T.red + "33" }}>
                {err}
              </div>
            )}
...
function Dashboard(props) {
  var txs = props.txs, station = props.station;
  var stTxs = txs.filter(function(t) { return !station || t.stationId === station; });
  var tday = todayStr();
  var todayTxs = stTxs.filter(function(t) { return t.weightIn && t.weightIn.startsWith(tday); });
  var pending = stTxs.filter(function(t) { return t.status === "pending_out"; });
  var todayDone = todayTxs.filter(function(t) { return t.status === "completed"; });
  var todayNet = todayDone.reduce(function(s, t) { return s + t.netKg; }, 0);

  var daily = Array.from({ length:14 }, function(_, i) {
    var d = new Date(Date.now() - (13 - i) * 86400000);
    var ds = d.toISOString().split("T")[0];
    var net = stTxs.filter(function(t) { return t.weightIn && t.weightIn.startsWith(ds) && t.status === "completed"; })
      .reduce(function(s, t) { return s + t.netKg; }, 0);
    return { day: d.toLocaleDateString("en-MY", { weekday:"short", day:"numeric" }), net: +(net / 1000).toFixed(2) };
  });

  var recent = stTxs.slice().sort(function(a, b) { return new Date(b.weightIn) - new Date(a.weightIn); }).slice(0, 8);

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:10 }}>
        <StatCard label="Today Tickets"     value={todayTxs.length}              sub="all status"        color={T.accent} icon="scale"   />
        <StatCard label="Pending Weigh-Out" value={pending.length}               sub="awaiting exit"     color={T.amber}  icon="queue"   />
        <StatCard label="Completed Today"   value={todayDone.length}             sub="today completed"   color={T.blue}   icon="check"   />
        <StatCard label="Today Net Weight"  value={(todayNet / 1000).toFixed(1) + "t"} sub={fmt(todayNet) + " kg"} color={T.purple} icon="chart" />
        <StatCard label="All Tickets"       value={stTxs.length}                 sub="total records"     color={T.mid}    icon="records" />
      </div>

      <Card>
        <div style={{ fontSize:11, color:T.mid, fontWeight:700, marginBottom:10, letterSpacing:0.5 }}>14-DAY NET FFB TONNAGE</div>
        <ResponsiveContainer width="100%" height={180}>
          <AreaChart data={daily} margin={{ top:5, right:10, left:-20, bottom:0 }}>
            ...
            <Area type="monotone" dataKey="net" stroke={T.accent} fill="url(#aGrad)" strokeWidth={2} dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </Card>

      <Card>
        <div style={{ fontSize:11, color:T.mid, fontWeight:700, marginBottom:10, letterSpacing:0.5 }}>RECENT TICKETS</div>
        <DataTable
          cols={[
...
function PendingQueue(props) {
  var txs = props.txs, stationId = props.stationId, onWeighOut = props.onWeighOut;
  var pending = txs.filter(function(t) { return t.status === "pending_out" && (!stationId || t.stationId === stationId); });
  var avgWait = pending.length
    ? Math.round(pending.reduce(function(s, t) { return s + (Date.now() - new Date(t.weightIn)) / 60000; }, 0) / pending.length)
    : 0;

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        <StatCard label="Pending Vehicles" value={pending.length} sub="awaiting weigh-out" color={T.amber} icon="queue" />
        <StatCard label="Avg Wait" value={pending.length ? avgWait + "m" : "--"} sub="minutes waiting" color={T.blue} icon="refresh" />
      </div>

      {pending.length === 0 ? (
        <Card style={{ textAlign:"center", padding:40 }}>
          <Ic d={IC.check} size={40} color={T.accent} />
          <div style={{ marginTop:10, color:T.accent, fontWeight:700 }}>No vehicles pending</div>
        </Card>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {pending.map(function(tx) {
            var waitMin = Math.round((Date.now() - new Date(tx.weightIn)) / 60000);
              ...
              <Card key={tx.id} style={{ padding:"14px 16px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:8 }}>
                    ...
                    <Btn label="WEIGH OUT" icon="weighOut" onClick={function() { onWeighOut(tx.id); }} small={true} variant="amber" />
                  </div>
                </div>
              </Card>
            );
...
function WeighOut(props) {
  var txs = props.txs, setTxs = props.setTxs;
  var stationId = props.stationId, preSelected = props.preSelected;
  ...
  var pending = txs.filter(function(t) { return t.status === "pending_out" && (!stationId || t.stationId === stationId); });
  var selTx = pending.find(function(t) { return t.id === selected; }) || null;
  var targetTare = selTx ? Math.round(selTx.grossKg * 0.55 + (Math.random() - 0.5) * 200) : 8000;

  function handleCapture(w) { setTareKg(w); }
  ...
  function handleComplete() {
    ...
    alert("Weigh-out complete!\nTicket: " + txId + "\nNet Weight: " + fmt(net) + " kg\nTicket Type: " + tt);
    setSelected(null);
    setTareKg(null);
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"1fr 380px", gap:18 }}>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <Card>
          <div style={{ fontSize:12, color:T.amber, fontWeight:700, marginBottom:12, letterSpacing:1, textTransform:"uppercase" }}>
            Select Vehicle for Weigh-Out ({pending.length})
          </div>
          {pending.length === 0 ? (
            <div style={{ textAlign:"center", padding:32, color:T.dim }}>No vehicles pending weigh-out</div>
          ) : (
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {pending.map(function(tx) {
                ...
                return (
                  <div key={tx.id}
                    onClick={function() { setSelected(tx.id); setTareKg(null); }}
                    style={{
                  ...
                  >
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <div>
                        <div style={{ fontFamily:"IBM Plex Mono,monospace", fontWeight:700, color:T.accent, fontSize:12 }}>{tx.id}</div>
                        <div style={{ fontSize:12, color:T.text, marginTop:1, display:"flex", alignItems:"center", gap:6 }}>
                          <span>{tx.plate}</span>
                        ...
                        <div style={{ fontSize:11, color:T.dim, marginTop:1 }}>In: {fmtD(tx.weightIn)} | Gross: {fmt(tx.grossKg)} kg</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontSize:10, color:T.dim }}>First Weight</div>
                        <div style={{ fontFamily:"IBM Plex Mono,monospace", color:T.blue, fontSize:13, fontWeight:700 }}>{fmt(tx.grossKg)} kg</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        {selTx && (
          <Card>
            <div style={{ fontSize:11, color:T.mid, fontWeight:700, marginBottom:12, letterSpacing:0.5 }}>TRANSACTION -- {selTx.id}</div>
...
["Supplier", selTx.supplierName], ["Product", selTx.product || "FFB"],
                ...
                return (
                  <div key={pair[0]}>
                    <div style={{ color:T.dim, fontSize:10, marginBottom:1 }}>{pair[0]}</div>
                    <div style={{ color:T.text, fontFamily:"IBM Plex Mono,monospace" }}>{pair[1]}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ marginBottom:12 }}>
              <div style={{ fontSize:11, color:T.mid, fontWeight:600, letterSpacing:0.5, textTransform:"uppercase", marginBottom:6 }}>TICKET TYPE *</div>
              <div style={{ display:"flex", gap:8 }}>
                {["Normal Invoice", "Cash Bill"].map(function(t) {
                  var isActive = ticketType === t;
                  return (
                    <button key={t} onClick={function() { setTicketType(t); }} style={{
            ...
            {tareKg && (
              <div style={{ marginBottom:12, padding:"10px 14px", background:T.accent + "11", border:"1px solid " + T.accent + "44", borderRadius:8, fontSize:13 }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:2 }}>
                  <div><div style={{ fontSize:10, color:T.dim, marginBottom:3 }}>First Weight</div><div style={{ fontFamily:"IBM Plex Mono,monospace", color:T.blue, fontWeight:700 }}>{fmt(selTx.grossKg)} kg</div></div>
                  <div><div style={{ fontSize:10, color:T.dim, marginBottom:3 }}>Second Weight</div><div style={{ fontFamily:"IBM Plex Mono,monospace", color:T.amber, fontWeight:700 }}>{fmt(tareKg)} kg</div></div>
                  <div><div style={{ fontSize:10, color:T.dim, marginBottom:3 }}>Net Weight</div><div style={{ fontFamily:"IBM Plex Mono,monospace", color:T.accent, fontWeight:800, fontSize:15 }}>{fmt(selTx.grossKg - tareKg)} kg</div></div>
                </div>
              </div>
            )}

            {tareKg && (
              <Btn label="CONFIRM WEIGH-OUT AND COMPLETE" icon="check" onClick={handleComplete} />
            )}
          </Card>
        )}
      </div>
      ...
      <div>
        {selTx ? (
          ...
          <Card style={{ textAlign:"center", padding:40 }}>
            ...
            <div style={{ marginTop:12, color:T.dim, fontSize:13 }}>Select a vehicle from the list to begin</div>
          </Card>
        )}
      </div>
    </div>
...
function TicketRecords(props) {
  var txs = props.txs, stationId = props.stationId, onPrint = props.onPrint;
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <Btn label="Back to Records" icon="records" onClick={function() { setViewTx(null); }} variant="ghost" small={true} />
          <Btn label="Print Preview" icon="print" onClick={function() { onPrint(viewTx); }} small={true} />
        </div>
        <Card>
              ...
              {[["Ticket No", viewTx.id], ["Station", viewTx.stationPrefix], ["Ticket Source", viewTx.ticketSource || "SCALE"], ["Customer Type", viewTx.customerType || "Registered Supplier"], ["Ticket Type", viewTx.ticketType || "Normal Invoice"], ["Product", viewTx.product || "Fresh Fruit Bunch (FFB)"], ["Status", viewTx.status === "completed" ? "Completed" : "Pending"]].map(function(p) {
                return (
                  <div key={p[0]}>
                    ...
                    <div style={{ color:T.text, fontFamily:"IBM Plex Mono,monospace", fontSize:13 }}>{p[1]}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, padding:14, background:T.bg, borderRadius:8, marginBottom:14 }}>
            {[["Gross Weight", fmt(viewTx.grossKg) + " kg", T.blue], ["Tare Weight", fmt(viewTx.tareKg) + " kg", T.amber], ["Net Weight", fmt(viewTx.netKg) + " kg", T.accent]].map(function(p) {
              return (
                <div key={p[0]} style={{ textAlign:"center" }}>
                  <div style={{ fontSize:10, color:T.dim, marginBottom:4 }}>{p[0]}</div>
                  <div style={{ fontSize:20, fontWeight:800, color:p[2], fontFamily:"IBM Plex Mono,monospace" }}>{p[1]}</div>
                </div>
              );
            })}
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            {[["Date/Time In", fmtD(viewTx.weightIn)], ["Date/Time Out", fmtD(viewTx.weightOut)], ["Operator", viewTx.operator]].map(function(p) {
              return (
                <div key={p[0]}>
                  <div style={{ fontSize:10, color:T.dim, marginBottom:2 }}>{p[0]}</div>
                  <div style={{ fontFamily:"IBM Plex Mono,monospace", color:T.text }}>{p[1]}</div>
                </div>
              );
            })}
          </div>
  ...
  var filtered = txs
    .filter(function(t) { return !stationId || t.stationId === stationId; })
    .filter(function(t) {
    ...
    .filter(function(t) {
      if (!q) return true;
      return t.id.toLowerCase().includes(q) || t.plate.toLowerCase().includes(q) || t.supplierName.toLowerCase().includes(q) || t.driverName.toLowerCase().includes(q);
    })
    .sort(function(a, b) { return new Date(b.weightIn) - new Date(a.weightIn); })
    .slice(0, 100);
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
        ...
        <div style={{ display:"flex", gap:4 }}>
          {["all", "pending", "completed"].map(function(f) {
            return (
              <button key={f} onClick={function() { setFilter(f); }} style={{
                background: filter === f ? T.accent + "22" : T.border,
                color: filter === f ? T.accent : T.mid,
      ...
      <Card>
        <div style={{ fontSize:11, color:T.dim, marginBottom:6 }}>{filtered.length} records found</div>
        <DataTable
          cols={[
            { key:"id",           label:"Ticket No",  render: function(v) { return <span style={{ fontFamily:"IBM Plex Mono,monospace", color:T.accent, fontWeight:700 }}>{v}</span>; } },
            { key:"supplierName", label:"Supplier" },
            ...
            { key:"id",           label:"Actions",    render: function(_, row) {
              return (
                <div style={{ display:"flex", gap:6 }}>
                  <Btn label="View"  icon="eye"   small={true} variant="ghost" onClick={function() { setViewTx(row); }} />
                  <Btn label="Print" icon="print" small={true} variant="ghost" onClick={function() { onPrint(row); }} />
                </div>
              );
            }},
          ]}
...
function CrudMod(props) {
  ...
  var items = props.items, setItems = props.setItems;
  var buildEmpty = props.buildEmpty, fields = props.fields, cols = props.cols;
  ...
  var filtered = items.filter(function(it) {
    return Object.values(it).some(function(v) { return String(v).toLowerCase().includes(q); });
  });

  function openNew() { setForm(buildEmpty()); setEditId(null); }
  function openEdit(it) { setForm(Object.assign({}, it, { estates: Array.isArray(it.estates) ? it.estates.join(", ") : (it.estates || "") })); setEditId(it.id); }
  ...
  function save() {
    ...
    if (typeof record.estates === "string") {
      record.estates = record.estates.split(",").map(function(e) { return e.trim(); }).filter(Boolean);
    }
    if (record.tare) record.tare = parseInt(record.tare);
    if (editId) {
      setItems(function(prev) { return prev.map(function(i) { return i.id === editId ? Object.assign({}, record, { id:editId }) : i; }); });
    } else {
      setItems(function(prev) { return prev.concat([Object.assign({}, record, { id:"id_" + Date.now() })]); });
    }
    setForm(null); setEditId(null);
  }

  function toggle(id) { setItems(function(prev) { return prev.map(function(i) { return i.id === id ? Object.assign({}, i, { active:!i.active }) : i; }); }); }
    ...
    render: function(_, row) {
      return (
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          <Btn label="Edit" icon="edit" small={true} variant="ghost" onClick={function() { openEdit(row); }} />
          <span onClick={function() { toggle(row.id); }} style={{ cursor:"pointer" }}>
            <Badge label={row.active ? "ACTIVE" : "INACTIVE"} color={row.active ? T.accent : T.red} />
          </span>
        </div>
      );
    },
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {form && (
        <Card style={{ borderColor: color + "44" }}>
          <div style={{ fontSize:12, fontWeight:700, color:color, marginBottom:12 }}>
            {editId ? "EDIT" : "NEW"} {title.toUpperCase()}
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:10 }}>
            {fields.map(function(f) {
              return (
                <FInput key={f.key} label={f.label} value={F[f.key] || ""}
          ...
          <div style={{ display:"flex", gap:8, marginTop:12 }}>
            ...
            <Btn label="Cancel" icon="x" onClick={function() { setForm(null); }} variant="ghost" />
          </div>
        </Card>
      )}

      <Card>
        <div style={{ fontSize:11, color:T.dim, marginBottom:8 }}>{filtered.length} records</div>
...
function SupplierMgmt(props) {
  return (
    <CrudMod
      title="Customer / Supplier" color={T.purple}
      items={props.suppliers} setItems={props.setSuppliers}
      buildEmpty={function() { return { name:"", code:"", address:"", contact:"", estates:"", active:true }; }}
      fields={[
        { key:"name",    label:"Company Name", required:true },
        { key:"code",    label:"Code (e.g. LMJ)", required:true },
        { key:"contact", label:"Contact No" },
        { key:"address", label:"Address" },
        { key:"estates", label:"Estates / Blocks (comma-separated)", full:true },
      ]}
      cols={[
        { key:"code",    label:"Code",     render: function(v) { return <span style={{ fontFamily:"IBM Plex Mono,monospace", color:T.accent, fontWeight:700 }}>{v}</span>; } },
        { key:"name",    label:"Customer / Supplier" },
        { key:"contact", label:"Contact" },
        { key:"address", label:"Address" },
        { key:"estates", label:"Estates",  render: function(v) { return (Array.isArray(v) ? v : []).slice(0, 3).map(function(e) { return <span key={e} style={{ marginRight:4 }}><Badge label={e} color={T.purple} /></span>; }); } },
      ]}
    />
  );
}
...
function VehicleMgmt(props) {
  ...
  return (
    <CrudMod
      title="Vehicle" color={T.blue}
      items={props.vehicles} setItems={props.setVehicles}
      buildEmpty={function() { return { plate:"", type:"Lorry", tare:"", rfid:"", driverId:"", stationId:"s1", active:true }; }}
      fields={[
        { key:"plate",    label:"Plate No", required:true },
        { key:"type",     label:"Type", options:[{value:"Lorry",label:"Lorry"},{value:"Truck",label:"Truck"},{value:"Pickup",label:"Pickup"},{value:"Trailer",label:"Trailer"}] },
        { key:"tare",     label:"Tare Weight (kg)", type:"number", required:true },
        { key:"rfid",     label:"RFID Tag", placeholder:"Optional" },
        { key:"driverId", label:"Default Driver", options:drivers.map(function(d) { return { value:d.id, label:d.name }; }) },
        { key:"stationId",label:"Station", options:STATIONS.map(function(s) { return { value:s.id, label:s.name + " (" + s.location + ")" }; }) },
      ]}
      cols={[
        { key:"plate",    label:"Plate",    render: function(v) { return <span style={{ fontFamily:"IBM Plex Mono,monospace", color:T.accent, fontWeight:700 }}>{v}</span>; } },
        { key:"type",     label:"Type" },
        { key:"tare",     label:"Tare (kg)", render: function(v) { return <span style={{ fontFamily:"IBM Plex Mono,monospace" }}>{fmt(v)}</span>; } },
        { key:"rfid",     label:"RFID",     render: function(v) { return v ? <Badge label={v} color={T.blue} /> : <span style={{ color:T.dim }}>--</span>; } },
        { key:"driverId", label:"Driver",   render: function(v) { var d = drivers.find(function(x) { return x.id === v; }); return d ? d.name : "--"; } },
      ]}
    />
  );
}
...
function DriverMgmt(props) {
  return (
    <CrudMod
      title="Driver"
      items={props.drivers} setItems={props.setDrivers}
      buildEmpty={function() { return { name:"", ic:"", phone:"", rfid:"", license:"", stationId:"s1", active:true }; }}
      fields={[
      ...
      cols={[
        ...
        { key:"stationId", label:"Station", render: function(v) { var s = STATIONS.find(function(x) { return x.id === v; }); return s ? s.name : v; } },
      ]}
    />
  );
}
...
function AppSettings(props) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16, maxWidth:640 }}>
          ...
          {[["System","PalmWeigh Pro"],["Version","v4.1 Phase 1 UAT"],["Company",COMPANY],["Logged In",props.currentUser ? props.currentUser.name : "--"],["Role",props.currentUser ? props.currentUser.role.toUpperCase() : "--"]].map(function(p) {
            return (
              <div key={p[0]} style={{ padding:"8px 10px", background:T.bg, borderRadius:6 }}>
                <div style={{ fontSize:10, color:T.dim, marginBottom:2 }}>{p[0]}</div>
                <div style={{ color:T.text, fontFamily:"IBM Plex Mono,monospace" }}>{p[1]}</div>
              </div>
            );
          })}
        </div>
      ...
      <Card>
        <div style={{ fontSize:12, color:T.accent, fontWeight:700, marginBottom:14 }}>STATION CONFIGURATION</div>
        <DataTable
          cols={[
            { key:"id",       label:"ID" },
            { key:"name",     label:"Station Name" },
            { key:"location", label:"Location" },
            { key:"prefix",   label:"Ticket Prefix", render: function(v) { return <Badge label={v} color={T.amber} />; } },
          ]}
          rows={STATIONS}
        />
      </Card>
      ...
      <Card>
        <div style={{ fontSize:12, color:T.accent, fontWeight:700, marginBottom:4 }}>USER ACCOUNTS</div>
        <div style={{ fontSize:11, color:T.dim, marginBottom:12 }}>UAT demo only -- production will use proper authentication</div>
        <DataTable
          cols={[
            { key:"name",     label:"Name" },
            { key:"username", label:"Username" },
            { key:"role",     label:"Role", render: function(v) { return <Badge label={v.toUpperCase()} color={v === "admin" ? T.accent : T.blue} />; } },
          ]}
          rows={props.users}
        />
      </Card>
      ...
      <Card style={{ borderColor:T.red + "44" }}>
        <div style={{ fontSize:12, color:T.red, fontWeight:700, marginBottom:6 }}>RESET DEMO DATA</div>
        <div style={{ fontSize:12, color:T.mid, marginBottom:12 }}>
          Restores all demo transactions, drivers, vehicles, and suppliers to default values. Cannot be undone.
        </div>
        {conf && (
          <div style={{ fontSize:12, color:T.amber, marginBottom:10, padding:"8px 10px", background:T.amber + "11", borderRadius:6, border:"1px solid " + T.amber + "33" }}>
            Click again to confirm. All current data will be overwritten.
          </div>
        )}
        <div style={{ display:"flex", gap:8 }}>
          <Btn label={conf ? "CONFIRM RESET" : "RESET DEMO DATA"} icon="refresh" onClick={doReset} variant="danger" />
          {conf && <Btn label="Cancel" icon="x" onClick={function() { setConf(false); }} variant="ghost" />}
        </div>
      </Card>
    </div>
  );
...
var PAYROLL_WORKERS = [
  { id:"W001", name:"Ahmad bin Yusof",     days:22, ot:8,  basic:1800, status:"OK"       },
  { id:"W002", name:"Azman bin Harun",     days:20, ot:14, basic:1800, status:"High OT"  },
  { id:"W003", name:"Kumar Selvam",        days:18, ot:6,  basic:1600, status:"Review"   },
  { id:"W004", name:"Raj Gopal",           days:22, ot:0,  basic:1700, status:"OK"       },
  { id:"W005", name:"Lee Chee Wai",        days:15, ot:4,  basic:1600, status:"Missing"  },
  { id:"W006", name:"Siti Aminah binti Johari", days:22, ot:18, basic:1800, status:"High OT"  },
  { id:"W007", name:"Faridah Othman",      days:21, ot:2,  basic:1500, status:"OK"       },
  { id:"W008", name:"Lim Boon Keat",       days:22, ot:6,  basic:1900, status:"OK"       },
  { id:"W009", name:"Selvakumar Raju",     days:19, ot:10, basic:1700, status:"Review"   },
  { id:"W010", name:"Nurul Hana Zainol",   days:0,  ot:0,  basic:1600, status:"Missing"  },
];
...
function AIPayroll() {
  ...
  var totalDays = PAYROLL_WORKERS.reduce(function(s, w) { return s + w.days; }, 0);
  var totalOT   = PAYROLL_WORKERS.reduce(function(s, w) { return s + w.ot; }, 0);
  var totalNet  = PAYROLL_WORKERS.reduce(function(s, w) { return s + calcPayroll(w).net; }, 0);
  var highOT    = PAYROLL_WORKERS.filter(function(w) { return w.ot >= 10; });
  var missing   = PAYROLL_WORKERS.filter(function(w) { return w.status === "Missing"; });
  var review    = PAYROLL_WORKERS.filter(function(w) { return w.status === "Review"; });

  var insights = [
    { color: T.amber, text: highOT.length + " worker" + (highOT.length !== 1 ? "s" : "") + " have high overtime (>= 10 hrs): " + highOT.map(function(w) { return w.name.split(" ")[0]; }).join(", ") },
    { color: T.red,   text: missing.length + " worker" + (missing.length !== 1 ? "s" : "") + " have missing attendance records and may require follow-up" },
    { color: T.purple,text: review.length + " worker salary requires supervisor review before finalising" },
    { color: T.accent,text: "Total estimated monthly payroll: RM " + Math.round(totalNet).toLocaleString("en-MY") },
    { color: T.blue,  text: "Average working days this period: " + (totalDays / PAYROLL_WORKERS.length).toFixed(1) + " days" },
  ];
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ padding:"10px 16px", background:T.amber + "18", border:"1px solid " + T.amber + "44", borderRadius:10, display:"flex", alignItems:"center", gap:10 }}>
        <Ic d={IC.alert} size={16} color={T.amber} />
        <span style={{ fontSize:12, color:T.amber, fontWeight:600 }}>Demo Preview Only -- final payroll rules will follow company salary policy.</span>
      </div>
      ...
      {tab === "summary" && (
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:10 }}>
            <StatCard label="Total Workers"         value={PAYROLL_WORKERS.length}           sub="on payroll list"     color={T.accent} icon="driver"   />
            <StatCard label="Attendance Records"    value={PAYROLL_WORKERS.filter(function(w){return w.days>0;}).length} sub="with valid records" color={T.blue}   icon="check"    />
            <StatCard label="Total Working Days"    value={totalDays}                         sub="across all workers"  color={T.mid}    icon="refresh"  />
            <StatCard label="Total Overtime Hours"  value={totalOT + "h"}                    sub="this period"         color={T.amber}  icon="chart"    />
            <StatCard label="Est. Monthly Payroll"  value={"RM " + Math.round(totalNet/1000).toFixed(1) + "k"} sub={"RM " + Math.round(totalNet).toLocaleString("en-MY")} color={T.accent} icon="scale" />
            <StatCard label="Pending Issues"        value={missing.length + review.length}   sub="require attention"   color={T.red}    icon="alert"    />
          </div>

          <Card>
            <div style={{ fontSize:11, color:T.mid, fontWeight:700, marginBottom:14, letterSpacing:0.5 }}>PAYROLL CALCULATION FORMULA</div>
            <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", padding:"14px 18px", background:T.bg, borderRadius:8, fontFamily:"IBM Plex Mono,monospace", fontSize:13 }}>
              ...
              <span style={{ color:T.dim }}>=</span>
              <span style={{ color:T.text, fontWeight:800, fontSize:15 }}>Net Salary</span>
            </div>
            <div style={{ marginTop:10, fontSize:11, color:T.dim, lineHeight:1.8 }}>
              <div>Overtime Rate: RM 10.50 per hour (demo rate)</div>
              <div>Deductions include unpaid leave, late deductions, and statutory contributions (demo only)</div>
              <div>Final values subject to HR approval and company salary policy</div>
            </div>
          </Card>
        </div>
      )}
      ...
      {tab === "workers" && (
        <Card>
          <div style={{ fontSize:11, color:T.mid, fontWeight:700, marginBottom:12, letterSpacing:0.5 }}>WORKER PAYROLL TABLE -- {PAYROLL_WORKERS.length} Workers</div>
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
              <thead>
                <tr style={{ borderBottom:"1px solid " + T.border }}>
                  {["Worker ID","Worker Name","Working Days","OT Hours","Basic Salary","OT Pay","Deductions","Est. Net Salary","Status"].map(function(h) {
                    return <th key={h} style={{ padding:"7px 10px", textAlign:"left", color:T.dim, fontWeight:600, fontSize:10, textTransform:"uppercase", letterSpacing:0.5, whiteSpace:"nowrap" }}>{h}</th>;
                  })}
                </tr>
              </thead>
              <tbody>
                {PAYROLL_WORKERS.map(function(w) {
                  ...
                  return (
                    <tr key={w.id} style={{ borderBottom:"1px solid " + T.border + "22" }}
                      onMouseEnter={function(e) { e.currentTarget.style.background = T.surface; }}
                      onMouseLeave={function(e) { e.currentTarget.style.background = "transparent"; }}>
                      <td style={{ padding:"8px 10px", color:T.dim, fontFamily:"IBM Plex Mono,monospace", fontSize:11 }}>{w.id}</td>
                      <td style={{ padding:"8px 10px", color:T.text, fontWeight:600 }}>{w.name}</td>
                      <td style={{ padding:"8px 10px", color:T.text, fontFamily:"IBM Plex Mono,monospace", textAlign:"center" }}>{w.days}</td>
                      <td style={{ padding:"8px 10px", fontFamily:"IBM Plex Mono,monospace", textAlign:"center", color: w.ot >= 10 ? T.amber : T.text }}>{w.ot}</td>
                      <td style={{ padding:"8px 10px", fontFamily:"IBM Plex Mono,monospace", color:T.blue }}>{"RM " + w.basic.toLocaleString("en-MY")}</td>
                      <td style={{ padding:"8px 10px", fontFamily:"IBM Plex Mono,monospace", color:T.accent }}>{"RM " + p.otPay.toFixed(2)}</td>
                      <td style={{ padding:"8px 10px", fontFamily:"IBM Plex Mono,monospace", color: p.deductions > 0 ? T.red : T.dim }}>{"RM " + p.deductions.toFixed(2)}</td>
                      <td style={{ padding:"8px 10px", fontFamily:"IBM Plex Mono,monospace", fontWeight:800, color:T.text }}>{"RM " + Math.round(p.net).toLocaleString("en-MY")}</td>
                      <td style={{ padding:"8px 10px" }}><Badge label={w.status} color={statusColor(w.status)} /></td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr style={{ borderTop:"2px solid " + T.border }}>
                  <td colSpan={4} style={{ padding:"10px", fontSize:12, color:T.mid, fontWeight:700 }}>TOTAL</td>
                  <td style={{ padding:"10px", fontFamily:"IBM Plex Mono,monospace", fontWeight:700, color:T.blue, fontSize:12 }}>{"RM " + PAYROLL_WORKERS.reduce(function(s,w){return s+w.basic;},0).toLocaleString("en-MY")}</td>
                  <td style={{ padding:"10px", fontFamily:"IBM Plex Mono,monospace", fontWeight:700, color:T.accent, fontSize:12 }}>{"RM " + PAYROLL_WORKERS.reduce(function(s,w){return s+calcPayroll(w).otPay;},0).toFixed(2)}</td>
                  <td style={{ padding:"10px", fontFamily:"IBM Plex Mono,monospace", fontWeight:700, color:T.red, fontSize:12 }}>{"RM " + PAYROLL_WORKERS.reduce(function(s,w){return s+calcPayroll(w).deductions;},0).toFixed(2)}</td>
                  <td style={{ padding:"10px", fontFamily:"IBM Plex Mono,monospace", fontWeight:800, color:T.accent, fontSize:13 }}>{"RM " + Math.round(totalNet).toLocaleString("en-MY")}</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <Card style={{ borderColor:T.purple + "44", background:T.card }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
              <div style={{ width:32, height:32, borderRadius:8, background:T.purple + "22", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <Ic d={IC.chart} size={16} color={T.purple} />
              </div>
              <div>
                <div style={{ fontSize:13, fontWeight:700, color:T.text }}>AI Payroll Insights</div>
                <div style={{ fontSize:11, color:T.dim }}>Rule-based analysis -- demo preview only</div>
              </div>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {insights.map(function(ins, i) {
                return (
                  <div key={i} style={{ display:"flex", gap:12, padding:"12px 14px", background:T.bg, borderRadius:8, borderLeft:"3px solid " + ins.color }}>
                    <div style={{ width:6, height:6, borderRadius:"50%", background:ins.color, marginTop:5, flexShrink:0 }}></div>
          ...
          <Card>
            <div style={{ fontSize:11, color:T.mid, fontWeight:700, marginBottom:12, letterSpacing:0.5 }}>WORKER STATUS BREAKDOWN</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
              {[
                ["OK",       PAYROLL_WORKERS.filter(function(w){return w.status==="OK";}).length,       T.accent ],
                ["High OT",  PAYROLL_WORKERS.filter(function(w){return w.status==="High OT";}).length,  T.amber  ],
                ["Review",   PAYROLL_WORKERS.filter(function(w){return w.status==="Review";}).length,   T.purple ],
                ["Missing",  PAYROLL_WORKERS.filter(function(w){return w.status==="Missing";}).length,  T.red    ],
              ].map(function(row) {
                return (
                  <div key={row[0]} style={{ padding:"14px", background:T.bg, borderRadius:8, textAlign:"center", borderTop:"3px solid " + row[2] }}>
                    <div style={{ fontSize:26, fontWeight:800, color:row[2], fontFamily:"IBM Plex Mono,monospace" }}>{row[1]}</div>
                    <div style={{ fontSize:11, color:T.dim, marginTop:4 }}>{row[0]}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ padding:"10px 16px", background:T.blue + "11", border:"1px solid " + T.blue + "33", borderRadius:8, fontSize:12, color:T.blue }}>
            <strong>Next Steps (Future Phases):</strong> Connect attendance records from biometric device, integrate with payroll software, enable manager approval workflow, generate EPF/SOCSO/EIS contribution reports.
          </div>
        </div>
      )}
    </div>
...
function FutureModules() {
  var phases = [
    {
      ...
      items: [
        { name:"Desktop Offline Mode",      desc:"Full offline operation with local database sync when connectivity restored." },
        { name:"Multi Collection Centres",  desc:"Support multiple weighbridge stations under one HQ account with centralised reporting." },
        { name:"Data Migration",            desc:"Import historical ticket records from existing systems into PalmWeigh." },
        { name:"HQ Reporting",              desc:"Consolidated reports across all stations viewable from headquarters." },
      ],
    },
    {
      phase: "Phase 3",
      ...
      items: [
        ...
        { name:"AI Payroll Assistant",      desc:"Rule-based AI to flag anomalies, suggest corrections, and summarise payroll." },
        { name:"Business Intelligence",     desc:"Charts, trends, and KPI dashboards for management decision making." },
      ],
    },
    {
      phase: "Phase 4",
      ...
      label: "Planned",
      items: [
        { name:"Real Weighbridge Integration", desc:"Live RS-232 / TCP-IP / Modbus connection to physical weighbridge indicator." },
        { name:"RFID Integration",           desc:"Automatic vehicle and driver identification via RFID reader at gate." },
        { name:"Camera Integration",         desc:"Capture vehicle photos at entry and exit for audit trail." },
        { name:"ANPR",                       desc:"Automatic number plate recognition to auto-fill vehicle details." },
      ],
    },
    {
      phase: "Phase 5",
      ...
      items: [
        { name:"Mobile App",                desc:"iOS and Android app for supervisors and management to monitor operations remotely." },
        { name:"Management Portal",         desc:"Web portal for owners and managers to view live operations and approve transactions." },
        { name:"WhatsApp Notifications",    desc:"Automated ticket confirmations, alerts, and daily summaries via WhatsApp." },
      ],
    },
  ];
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ padding:"12px 16px", background:T.surface, border:"1px solid " + T.border, borderRadius:10, display:"flex", alignItems:"flex-start", gap:12 }}>
        <Ic d={IC.alert} size={18} color={T.amber} />
        <div>
          <div style={{ fontSize:12, fontWeight:700, color:T.amber, marginBottom:4 }}>Notice</div>
          <div style={{ fontSize:12, color:T.mid, lineHeight:1.6 }}>
            These modules are not included in the current demo and are shown for future expansion planning only. All features listed below require separate development phases, timelines, and agreements.
          </div>
        </div>
      </div>
        ...
        return (
          <div key={ph.phase}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
              <div style={{ fontSize:13, fontWeight:800, color:ph.color, fontFamily:"IBM Plex Mono,monospace" }}>{ph.phase}</div>
              ...
              {ph.items.map(function(item) {
                return (
                  <div key={item.name} style={{
                    ...
                    <div style={{ fontSize:12, fontWeight:700, color:T.text, marginBottom:6 }}>{item.name}</div>
                    <div style={{ fontSize:11, color:T.dim, lineHeight:1.6 }}>{item.desc}</div>
                    <div style={{ marginTop:10 }}>
      ...
      <div style={{ padding:"12px 16px", background:T.blue + "11", border:"1px solid " + T.blue + "33", borderRadius:8, fontSize:12, color:T.blue, lineHeight:1.7 }}>
        <strong>Current Demo Scope:</strong> Login, Weigh In (Registered Supplier and Walk-In Customer), Pending Queue, Weigh Out, Ticket Records, Print Preview, Customer/Supplier Management, Vehicle Management, Driver Management, Settings, and AI Payroll Preview.
      </div>
    </div>
  );
}
...
export default function PalmWeighPro() {
  ...
  var pending = txs.filter(function(t) { return t.status === "pending_out" && t.stationId === station; }).length;

  function goWeighOut(txId) { setQueueTarget(txId); setPage("weighOut"); }

  if (!currentUser) return <Login users={users} onLogin={setCurrentUser} />;

  var staNow = STATIONS.find(function(s) { return s.id === station; });
  var pageTitle = (NAV.find(function(n) { return n.id === page; }) || {}).label || "";
  ...
  return (
    <div>
      <style>{[
        ...
        "body{background:" + T.bg + ";font-family:'IBM Plex Sans',sans-serif;color:" + T.text + ";}",
      ...
      {printTx && <PrintPreview tx={printTx} onClose={function() { setPrintTx(null); }} />}
      ...
      <div style={{ display:"flex", height:"100vh", overflow:"hidden" }}>
        ...
        <div style={{
          ...
          display:"flex", flexDirection:"column", transition:"width .2s", flexShrink:0,
        }}>
          <div style={{ padding: collapsed ? "12px 0" : "12px 12px", borderBottom:"1px solid " + T.border, display:"flex", alignItems:"center", gap:8, justifyContent: collapsed ? "center" : "space-between" }}>
            {!collapsed && (
              <div>
            ...
            <button onClick={function() { setCollapsed(function(c) { return !c; }); }}
              ...
              <Ic d={collapsed ? "M9 18l6-6-6-6" : "M15 18l-6-6 6-6"} size={16} color={T.dim} />
            </button>
          </div>

          {!collapsed && (
            <div style={{ padding:"8px 10px", borderBottom:"1px solid " + T.border }}>
              <div style={{ fontSize:9, color:T.dim, marginBottom:3, letterSpacing:0.5 }}>ACTIVE STATION</div>
              <select value={station} onChange={function(e) { setStation(e.target.value); }} style={Object.assign({}, IS, { fontSize:11, padding:"5px 8px" })}>
                {STATIONS.map(function(s) { return <option key={s.id} value={s.id}>{s.name} ({s.prefix})</option>; })}
              </select>
            </div>
          )}
          ...
          <nav style={{ flex:1, overflowY:"auto", padding:"6px 0" }}>
            {NAV.map(function(item) {
              ...
              var hasBadge = (item.id === "weighOut" || item.id === "queue") && pending > 0;
              return (
                <button key={item.id}
                ...
                >
                  <div style={{ position:"relative", lineHeight:0 }}>
                    <Ic d={IC[item.icon]} size={15} color={active ? T.accent : T.mid} />
                    {hasBadge && !collapsed && (
                      <div style={{ position:"absolute", top:-3, right:-3, width:6, height:6, borderRadius:"50%", background:T.amber }} />
                    )}
                  </div>
                  {!collapsed && item.label}
                  {!collapsed && hasBadge && <Badge label={pending} color={T.amber} />}
                </button>
              );
            })}
          </nav>
          ...
          {!collapsed && (
            <div style={{ padding:"8px 10px", borderTop:"1px solid " + T.border }}>
              <div style={{ fontSize:10, color:T.dim, marginBottom:6 }}>{currentUser.name} - {currentUser.role.toUpperCase()}</div>
              <button onClick={function() { setCurrentUser(null); }}
                style={{ background:"none", border:"none", cursor:"pointer", color:T.dim, fontSize:11, display:"flex", alignItems:"center", gap:5, fontFamily:"IBM Plex Sans,sans-serif" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          <div style={{ height:48, background:T.surface, borderBottom:"1px solid " + T.border, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 18px", flexShrink:0 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ fontSize:13, fontWeight:700, color:T.text }}>{pageTitle}</div>
              {staNow && <Badge label={staNow.prefix} color={T.accent} />}
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:12, fontSize:11, color:T.dim }}>
              {pending > 0 && <Badge label={pending + " pending"} color={T.amber} />}
          ...
          <div style={{ background:"#0d1829", borderBottom:"1px solid " + T.border, padding:"8px 18px", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <Badge label="UAT DEMO" color={T.amber} />
              <span style={{ fontSize:11, color:T.dim }}>PalmWeigh Pro -- UAT Demonstration Version. Data is stored locally and can be reset in Settings at any time.</span>
            </div>
            <span style={{ fontSize:10, color:T.dim, fontFamily:"IBM Plex Mono,monospace" }}>For workflow testing and evaluation purposes only.</span>
          </div>
          <div style={{ flex:1, overflowY:"auto", padding:18 }}>
            ...
            {page === "queue" && (
              <PendingQueue txs={txs} stationId={station} onWeighOut={goWeighOut} />
            )}
            {page === "weighOut" && (
              <WeighOut txs={txs} setTxs={setTxs} stationId={station} preSelected={queueTarget} />
            )}
            {page === "records" && (
              <TicketRecords txs={txs} stationId={station} onPrint={setPrintTx} />
            )}
            {page === "suppliers" && (
              <SupplierMgmt suppliers={suppliers} setSuppliers={setSuppliers} />
            )}
            {page === "vehicles" && (
              <VehicleMgmt vehicles={vehicles} setVehicles={setVehicles} drivers={drivers} />
            )}
            {page === "drivers" && (
